<template>
<div class="absolute inset-0 padding__standard overflow-auto overflow-x-hidden bg-slate ">
  <div class="gap-[40px]  grid grid-cols-1 grid-rows-[1fr,auto,auto]">
    <section class="flex flex-col gap-[20px]  md:gap-[24px]  ">
      <Icon name="backArrow" color="white"
        class=" -ml-[25px] md:-ml-[50px] -mt-[40px] md:mt-0 mb-[30px]  cursor-pointer mb-4" @click="emit('back')" />
      <header>
    ...
    <section
      class="relative flex items-center gap-8 h-[75vw] w-[100vw] -ml-[43px] pl-[43px] md:-ml-[86px] md:pl-[86px]  overflow-x-auto  pr-[40px] md:pr-[80px] hide-scroll">
      <ProjectExample v-for="example in examples" :key="example.url" :example="example"
