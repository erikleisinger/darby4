<template>
  <div
    class="relative border-[4px] border-solid rounded-sm transition-all grid grid-cols-[1fr,auto] group"
    :class="
      errors && dirty
        ? 'border-red'
  ...
  >
    ...
    <input
      ...
      @input="dirty = true"
      @blur="dirty = true"
      class="bg-slate focus:outline-none focus-within:outline-none p-2 h-[50px]"
    />
    ...
    <div class="m-auto px-4" v-if="value">
      <Icon
        ...
        color="gold"
        height="30px"
        class="cursor-pointer hover:stroke-[white]"
        @click="value = ''"
      />
    </div>
    <Typography
